...
class AdvancedTournamentGenerator:
    ...
    def generate_round_matches(self, players, courts, round_index, avoid_mm_vs_ff=True, use_rating_balance=True, rating_factor=3, manual_sit_outs=None):
        ...
        for court_num in range(courts):
            ...
            for combo in itertools.combinations(available_players, 4):
                ...
                for teamA in itertools.combinations(combo, 2):
                    ...
                    score = score_match_quality(teamA, teamB)
                    
                    if score < best_score:
                        best_score = score
                        best_match = (combo, teamA, teamB)
            
            if best_match is None:
                return None, f"Could not form valid match for court {court_num + 1}"
            
            combo, teamA, teamB = best_match
...
def generate_tournament():
    try:
        ...
        if use_defaults:
            ...
            print(f"DEBUG: Player names: {[p['name'] for p in players_list]}")
        else:

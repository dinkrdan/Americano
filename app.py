...
class AdvancedTournamentGenerator:
    ...
    def generate_round_matches(self, players, courts, round_index, avoid_mm_vs_ff=True, use_rating_balance=True, rating_factor=3):
        """Generate matches for a single round using sophisticated algorithms - FIXED VERSION"""
        ...
        # FIXED: Use a proper backtracking algorithm to assign players to matches
        chosen_matches = []
        used_players = set()

        def form_matches_fixed(available_players, match_count):
            """Fixed match formation with proper player tracking"""
            if match_count == courts:
                return True
            if len(available_players) < 4:
                return False

            # Get all possible 4-player combinations from available players
            combos = list(itertools.combinations(available_players, 4))
            random.shuffle(combos)

            for combo in combos:
                # Verify no player overlap with already used players
                if any(p in used_players for p in combo):
                    continue
                    
                # Try all possible team splits for this 4-player combo
                ...
                if use_rating_balance:
                    total_rounds = max(1, round_index + 1)
                ...
                for (teamA, teamB), score in possible_team_configs:
                    # Add this match and mark players as used
                    chosen_matches.append((teamA, teamB))
                    for player in combo:
                        used_players.add(player)
                    
                    # Create new available players list without the used players
                    new_available = [p for p in available_players if p not in combo]
                    
                    print(f"DEBUG: Match {match_count + 1}: {teamA} vs {teamB}")
                    print(f"DEBUG: Remaining players: {len(new_available)}")
                    
                    # Recursively try to form remaining matches
                    if form_matches_fixed(new_available, match_count + 1):
                        return True
                    
                    # Backtrack: remove this match and unmark players
                    chosen_matches.pop()
                    for player in combo:
        ...
        # Start the match formation process
        success = form_matches_fixed(player_names, 0)
        
        if not success:
            return None, f"Cannot form suitable matches in round {round_index + 1}"

        print(f"DEBUG: Successfully formed {len(chosen_matches)} matches")
        
        # Verify no player appears twice
        all_playing_players = []
        for teamA, teamB in chosen_matches:
            all_playing_players.extend(teamA)
            all_playing_players.extend(teamB)
        
        if len(all_playing_players) != len(set(all_playing_players)):
            duplicates = [p for p in all_playing_players if all_playing_players.count(p) > 1]
            return None, f"ERROR: Duplicate player assignments detected: {duplicates}"
        
        print(f"DEBUG: Verification passed - no duplicate players")
...
def generate_tournament():
    try:
        ...
        # Store configuration in session with fresh generator
        global tournament_gen
        tournament_gen = AdvancedTournamentGenerator()  # Reset for new tournament
...
def advance_round():
    try:
        ...
        # FIXED: Use global tournament_gen instance with existing state
        global tournament_gen
